...
public class AdoptionController {
    ...
    //查全部
    @GetMapping
    public Result selectAllAdoption(@RequestParam("pageNum") int pageNum,
                                    @RequestParam("pageSize") int pageSize) {
        IPage<Adoption> page = adoptionService.selectAllAdoptionPage(pageNum,pageSize);
        PageBean<Adoption> pageBean= new PageBean<>(page.getTotal(),page.getRecords());
    ...
    public Result updateAdoption(@RequestBody Adoption adoption) {
        ...
        if ( result > 0 ){
            ...
            return Result.error("修改寄养信息失败");
        }
    }
    //模糊查询
    @GetMapping("search")
    public Result searchAdoption(@RequestParam String keyword) {
        List<Adoption> result = adoptionService.findAdoptionByKeyword(keyword);
        if (result != null){
            return Result.success(result);
        }else {
